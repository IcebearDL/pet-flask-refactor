...
class InterviewTable extends React.Component {
  ...
  render() {
    ...
    const columns = [{
      ...
      align: 'center',
      render: status => {
        if (status === 0) return '死亡'
        else if (status === 1) return '存活'
        else if (status === 2) return '失联'
      }
    }, {
      title: '死亡日期',
      dataIndex: 'die_time',
    ...
    return (
      <div className={styles.menu_div}>
        ...
        <div className={styles.menu_content}>
          <Button type="primary" onClick={() => this.handleEditModel({ interview_id: '' })}>添加</Button>
          <Table
