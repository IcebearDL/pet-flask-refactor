...
class InterviewTable extends React.Component {
  ...
  render() {
    ...
    const tableLoading = this.props.loading.effects[
      'crf_interview/fetchInterviewTable'
    ]
    const submitLoading = this.props.loading.effects[
      'crf_interview/modifyInterview'
    ]
    ...
    return (
      <div className={styles.menu_div}>
        ...
        <div className={styles.menu_content}>
          <Button
            type="primary"
            onClick={() => this.handleEditModel({ interview_id: '' })}
          >
            添加
          </Button>
          <Table
            ...
            >
              <Form.Item label="随访日期">
                {getFieldDecorator('interview_time', {
                  initialValue: record.interview_time
                    ? moment(record.interview_time, 'YYYY-MM-DD')
                    : null
              ...
              <Form.Item label="死亡原因">
                {getFieldDecorator('die_reason', {
                  ...
                  <Radio.Group
                    onChange={e => this.handleStateChange('die_reason', e)}
                  >
              ...
              <Form.Item label="OS随访证明文件收集形式">
                {getFieldDecorator('OS_method', {
                  ...
                  <Radio.Group
                    onChange={e => this.handleStateChange('OS_method', e)}
                  >
              ...
              <Form.Item label="生存状态确认日期">
                {getFieldDecorator('status_confirm_time', {
                  initialValue: record.status_confirm_time
                    ? moment(record.status_confirm_time, 'YYYY-MM-DD')
                    : null
                })(<DatePicker format="YYYY-MM-DD" />)}
              </Form.Item>
              <Form.Item label="最后一次联系上并确认患者仍存活的日期">
                {getFieldDecorator('last_time_survival', {
                  initialValue: record.last_time_survival
                    ? moment(record.last_time_survival, 'YYYY-MM-DD')
                    : null
                })(<DatePicker format="YYYY-MM-DD" />)}
              </Form.Item>
